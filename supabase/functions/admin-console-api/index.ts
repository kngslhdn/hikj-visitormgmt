...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(action==='distribution_history'){const n=limitOf(url.searchParams.get('limit'),200,5000),search=(url.searchParams.get('q')||'').trim().toLowerCase(),from=url.searchParams.get('from'),to=isoEnd(url.searchParams.get('to')),status=url.searchParams.get('status');let q=sb.from('package_distribution_history').select('*').order('distributed_at',{ascending:false}).limit(n);if(from)q=q.gte('distributed_at',from);if(to)q=q.lt('distributed_at',to);if(status)q=q.eq('status',status);const {data,error}=await q;if(error)throw error;const rows=(data||[]).filter(row=>{if(!search)return true;const haystack=[row.package_number,row.recipient_name,row.registered_recipient_name,row.company_name,row.courier_name,row.security_hand_over].join(' ').toLowerCase();return haystack.includes(search)});return json(req,{data:rows.slice(0,n)})}
