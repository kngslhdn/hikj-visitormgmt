...
async function findExitEntry(name:string,pass:string){
 const nameKey=normText(name),passKey=normText(pass);const {data,error}=await supabase.from('visitor_entries').select('id,visitor_id,visitor_name,pass_vest_number,entry_at').is('exit_id',null).order('entry_at',{ascending:false}).limit(200);if(error)throw error;return (data||[]).find(e=>(nameKey&&normText(e.visitor_name)===nameKey)||(passKey&&normText(e.pass_vest_number)===passKey))||null;
}
...
Deno.serve(async req=>{
 ...
 try{
  ...
  if(type==='visitor_entry'||type==='visitor_exit'){
   ...
   if(type==='visitor_exit'){matchedExitEntry=await findExitEntry(name,pass);if(!matchedExitEntry)return json({ok:false,error:'Visitor not found: Name or Pass/Vest Number does not match any visitor currently inside the hotel.'},404);visitorId=matchedExitEntry.visitor_id;}else visitorId=await visitorIdentity(name,mobile,company,clean(body.category)||null);
  }
  ...
  if(type==='visitor_entry'){
   ...
   const exitCompany=clean(entry?.visitor_company_name||'');const exitName=clean(entry?.visitor_name||name);
   const {data:ex,error}=await supabase.from('visitor_exits').insert({submission_id:submission.id,visitor_id:visitorId,entry_id:entry?.id||null,visitor_name:exitName,company_name:exitCompany||null,pass_vest_number:pass,security_officer_name:clean(val(body,'security_officer_name','security')),exit_at:timestamp(body.exit_at||body.datetime)}).select('id').single();if(error)throw error;if(entry?.id)await supabase.from('visitor_entries').update({exit_id:ex.id}).eq('id',entry.id);
  }else if(type==='key_borrowing'){
   const key=clean(body.key_number||body.keyNumber);const quantity=Number(body.quantity||body.qty||1);if(!key)return json({error:'Please enter Key Number.'},400);if(!Number.isInteger(quantity)||quantity<1)return json({error:'Quantity of Keys must be a whole number greater than 0.'},400);
   const {data:outstanding,error:oe}=await supabase.from('key_borrowings').select('id,borrower_name,quantity').eq('key_number',key).is('return_id',null).order('borrowed_at',{ascending:false}).limit(1).maybeSingle();if(oe)throw oe;if(outstanding)return json({ok:false,error:`Key ${key} is currently borrowed by ${outstanding.borrower_name}. Please return the key before borrowing it again.`},409);
   ...
   const key=clean(body.key_number||body.keyNumber);const quantity=Number(body.quantity||body.qty||1);if(!key)return json({error:'Please enter Key Number.'},400);if(!Number.isInteger(quantity)||quantity<1)return json({error:'Quantity of Keys must be a whole number greater than 0.'},400);
   const {data:borrowing,error:be}=await supabase.from('key_borrowings').select('id,key_number,key_description,quantity,borrower_name,department').eq('key_number',key).is('return_id',null).order('borrowed_at',{ascending:false}).limit(1).maybeSingle();if(be)throw be;if(!borrowing)return json({ok:false,error:`Key ${key} is not currently borrowed. Return rejected.`},409);
   if(quantity>Number(borrowing.quantity))return json({ok:false,error:`Return quantity (${quantity}) cannot exceed borrowed quantity (${borrowing.quantity}) for key ${key}.`},400);
   if(quantity!==Number(borrowing.quantity))return json({ok:false,error:`Partial key return is not supported. Please return all ${borrowing.quantity} key(s) for ${key}.`},400);
   const {data:ret,error}=await supabase.from('key_returns').insert({submission_id:submission.id,borrowing_id:borrowing.id,return_name:clean(val(body,'return_name','returnName')),department:clean(body.department)||borrowing.department,key_number:key,key_description:clean(body.key_description||body.description)||borrowing.key_description,quantity,security_officer_name:clean(val(body,'security_officer_name','security')),returned_at:timestamp(body.returned_at||body.datetime)}).select('id').single();if(error)throw error;if(borrowing.id)await supabase.from('key_borrowings').update({return_id:ret.id}).eq('id',borrowing.id);
  }else{
  ...
  await supabase.from('submissions').update({status:'completed'}).eq('id',submission.id);return json({ok:true,submission_id:submissionId});
