...
async function whatsapp(token){
 ...
 const waSave=$('waSave'),waHead=$('sContent').querySelector('.s-head'),waActions=waSave.parentElement;waHead.appendChild(waSave);waActions.remove();
...
async function operations(token){
 const r=await req('settings');if(!current(token))return;const o=r.settings?.operations||{};const fields=[['visitor_entry_enabled','Visitor Entry Registration'],['visitor_exit_enabled','Visitor Exit Registration'],['key_borrowing_enabled','Key Borrowing'],['key_return_enabled','Key Return'],['package_registration_enabled','Package Registration'],['package_distribution_enabled','Package Distribution']];
