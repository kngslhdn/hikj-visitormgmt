 ...
 rows.forEach((p,i)=>{
   ...
   const btn=document.querySelector('[data-prop-save="'+i+'"]');btn.onclick=async()=>{
     ...
     try{
       ...
       await req('save_property','POST',payload);msg('propMsg_'+i,'Property settings saved successfully');
...
async function render(tab='whatsapp',token=null){if(!current(token))return;
 style();$('aPage').innerHTML='<div class="a-head"><div><div class="a-kicker">SECUREOPS SECURITY</div><h1>Settings</h1><p>System configuration, access control and key inventory</p></div></div><div class="a-tabs">'+['property','whatsapp','admins','keys','operations','audit'].map((x,i)=>`<button class="${x===tab?'active':''}" data-stab="${x}">${x==='property'?'Property':x==='whatsapp'?'WhatsApp':x==='admins'?'Admin Users':x==='keys'?'Key Assets':x==='operations'?'System / Operations':'Audit Log'}</button>`).join('')+'</div><div id="sContent" class="s-wrap"></div>';
